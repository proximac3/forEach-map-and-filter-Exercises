...
function showFirstAndLast(arr){
    ...
    for (let i = 0; i < arr.length; i++){
        results.push(`${arr[i][0]}${arr[i].split('').slice(-1)}`)
    }
    return results
}
...
function vowelCount(str){
    ...
    str.split('').forEach(elem => {
        let letter = elem.toLowerCase()
        if (vowels.includes(letter)) {
            if (result.hasOwnProperty(letter)) {
                let counter = result[letter]
...
/**
an object with the key as the vowel and the value as the number of times the vowel occured.
...
function removeVowels(str) {
    ...
    let result = str.split('').filter(elem => {
